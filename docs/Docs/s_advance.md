...
`block` will not contain such a transaction, because it is not broadcast on the node network, it is the data generated by vm, each node that gets the smart contract transaction to execute, save the execution result on its own node, when a district After all the contract transactions in the block have been executed, a `stateRoot` will be generated, and this `stateRoot` will be broadcast in `block`.

The contract nuls asset transfer transaction is not signed. This type of transaction is generated when each node verifies the block. In addition, such transactions are not in the range of broadcast data, and need to be retrieved from another interface. Please refer to the following details for detailed data. Interface** to get the _serialized string_data of the transaction.
