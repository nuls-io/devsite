...
At the same time, the contract has an initialization method that will be executed immediately after the contract is successfully created. Therefore, if the user has calculated in the initialization method, please give enough GAS fee. If the GAS is insufficient, the initialization will fail. The implicated contract creation failed.
