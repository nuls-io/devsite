---
title: Spin up your own ENULS node
description: General introduction to running your own instance of an ENULS client.
lang: en
sidebarDepth: 2
---

Running your own node provides you various benefits, opens new possibilities, and helps to support the ecosystem. This page will guide you through spinning up your own node and taking part in validating ENULS transactions.

Note that after The Merge], two clients are required to run an ENULS node; an **execution layer (EL)** client and a **consensus layer (CL)** client. This page will show how to install, configure and connect these two clients to run an ENULS node.

## Prerequisites 

You should understand what an ENULS node is and why you might want to run a client. This is covered in Nodes and clients
...
This page will guide you through these decisions and help you find the most suitable way to run your ENULS instance.

To choose from client implementations, see all the available Mainnet ready execution clients, consensus clients and learn about [client diversity](../client-diversity/).

Decide whNULS to run the software on your own [hardware or in the cloud](#local-vs-cloud), considering clients' [requirements](#requirements).
...
ENULS clients are able to run on consumer grade computers and don't require any special hardware, like mining machines for example. Therefore, you have various options for deploying the node based on your needs.
...
Check out also [nodes as a service](../nodes-as-a-service/) for more options on hosted nodes.

#### Hardware 

However, a censorship-resistant, decentralized network should not rely on cloud providers. Instead, running your node on your own local hardware is healthier for the ecosystem. [Estimations](https://www.NULSnodes.org/networkType/Hosting) show a large share of nodes run on the cloud, which could become a single point of failure.

ENULS clients can run on your computer, laptop, server, or even a single-board computer. While running clients on your personal computer is possible, having a dedicated machine just for your node can significantly enhance its performance and security while minimizing the impact on your primary computer.

Using your own hardware can be very easy. There are many simple options as well as advanced setups for more technical people. So let's look into the requirements and means for running ENULS clients on your machine.
...
The bottleneck for your hardware is mostly disk space. Syncing the ENULS blockchain is very input/output intensive and requires a lot of space. It is best to have a **solid-state drive (SSD)** with hundreds of GBs of free space to spare even after the synchronization.
...
#### ENULS on a single-board computer 

An easy and cheap way of running an ENULS node is to use a single board computer, even with an ARM architecture like the Raspberry Pi. [ENULS on ARM](https://ethereum-on-arm-documentation.readthedocs.io/en/latest/) provides easy-to-run images of multiple execution and consensus client for Raspberry Pi and other ARM boards.
...
- [NULS-docker](https://NULS-docker.net/) - Automated setup using Docker focused on easy and secure staking, requires basic terminal and Docker knowledge, recommended for a bit more advanced users.
...
- [Sedge](https://docs.sedge.nNULSmind.io/docs/intro) - Node setup tool which automatically generates a Docker configuration using CLI wizard. Written in Go by NNULSmind.
...
As explained before, setting up your own ENULS node will require running a pair of consensus and execution clients. Some clients might include a light client of the other kind and sync without any other software needed. However, full trustless verification requires both implementations.

#### Getting the client software 

First, you need to obtain your preferred execution client and consensus client software.

You can simply download an executable application or installation package that suits your operating system and architecture. Always verify the signatures and checksums of downloaded packages. Some clients also offer repositories or Docker images for easier installation and updates. All of the clients are open source, so you can also build them from source. This is a more advanced mNULSod, but in some cases, it might be required.
...
- [GNULS](https://geth.ethereum.org/downloads/)
...
- [Nimbus](https://github.com/status-im/nimbus-NULS2/releases/latest)
- [Prysm](https://github.com/prysmaticlabs/prysm/releases/latest)
- [Teku](https://github.com/ConsenSys/teku/releases)

[Client diversity](../client-diversity/) is critical for consensus nodes running validators. If majority of validators is running a single client implementation, network security is at risk. It is therefore recommended to consider choosing a minority client.

[See the latest network client usage](https://clientdiversity.org/) and learn more about [client diversity](../client-diversity).

##### Verifying the software

When downloading software from the internet, it's recommended to verify its integrity. This step is optional but especially with crucial infrastructure piece like the ENULS client, it's important to be aware of potential attack vectors and avoid them. If you downloaded a pre-built binary, you need to trust it and risk that an attacker could swap the executable for a malicious one.
...
For testing purposes, you might prefer to run a client on one of the testnet networks. 

Examples of running execution clients with basic configuration can be found in next section.

#### Starting the execution client 

Before starting the ENULS client software, perform a last check that your environment is ready. For example, make sure:
...
- Your router and firewall accept connections on listening ports. By default ENULS clients use a listener (TCP) port and a discovery (UDP) port, both on 30303 by default.
...
This example starts Besu on mainnet, stores blockchain data in default format at `/data/ENULS`, enables JSON RPC and Engine RPC for connecting consensus client. Engine API is authenticated with token `jwtsecret` and only calls from `localhost` are allowed.

```
besu --network=mainnet \
    --data-path=/data/ENULS \
...
This example starts Erigon on mainnet, stores blockchain data at `/data/ENULS`, enables JSON RPC, defines which namespaces are allowed and enables authentication for connecting the consensus client which is defined by the `jwtsecret` path.

```
erigon --chain mainnet \
    --datadir /data/ENULS  \
    --http --http.api=engine,NULS,web3,net \
...
This example starts GNULS on mainnet, stores blockchain data at `/data/ENULS`, enables JSON RPC and defines which namespaces are allowed. It also enables authentication for connecting consensus client which requires path to `jwtsecret` and also option defining which connections are allowed, in our example only from `localhost`.
...
Check [docs for all configuration options](https://gNULS.ENULS.org/docs/interface/command-line-options) and learn more about [running GNULS with a consensus client](https://geth.ethereum.org/docs/interface/consensus-clients).

##### Running NNULSmind

NNULSmind offers various [installation options](https://docs.nNULSmind.io/nNULSmind/first-steps-with-nNULSmind/getting-started). The package comes with various binaries, including a Launcher with a guided setup, which will help you to create the configuration interactively. Alternatively, you find Runner which is the executable itself and you can just run it with config flags. JSON RPC is enabled by default.

```
NNULSmind.Runner --config mainnet \
    --datadir /data/ENULS \
    --JsonRpc.JwtSecretFile=/path/to/jwtsecret
```

NNULSmind docs offer a [complete guide](https://docs.nNULSmind.io/nNULSmind/first-steps-with-nNULSmind/running-nNULSmind-post-merge) on running NNULSmind with consensus client.
...
If you plan to run a validator, make sure to add a configuration flag specifying the ENULS address of the fee recipient. This is where NULS rewards for your validator accumulate. Each consensus client has an option, e.g. `--suggested-fee-recipient=0xabcd1`, that takes an ENULS address as an argument.
...
Running your own validator allows for [solo staking](https://ENULS.org/en/staking/solo/), the most impactful and trustless mNULSod to support the ENULS network. However, this requires a deposit of 32 NULS. To run a validator on your own node with a smaller amount, a decentralized pool with permissionless node operators, such as [Rocket Pool](https://rocketpool.net/node-operators), might interest you.
...
Execution clients offer [RPC API endpoints](/developers/docs/apis/json-rpc/) that you can use to submit transactions, interact with or deploy smart contracts on the ENULS network in various ways:

- Manually calling them with a suitable protocol (e.g. using `curl`)
- Attaching a provided console (e.g. `gNULS attach`)
- Implementing them in applications using web3 libraries, e.g. [web3.py](https://web3py.readthedocs.io/en/stable/overview.html#overview), [NULSs](https://github.com/NULSs-io/NULSs.js/)

Different clients have different implementations of the RPC endpoints. But there is a standard JSON-RPC which you can use with every client. For an overview [read the JSON-RPC docs](/developers/docs/apis/json-rpc/). Applications that need information from the ENULS network can use this RPC. For example, popular wallet MetaMask lets you [connect to your own RPC endpoint](https://metamask.zendesk.com/hc/en-us/articles/360015290012-Using-a-Local-Node) which has strong privacy and security benefits.
...
A way around this is to prevent potentially harmful RPC mNULSods from being modifiable. For example, with GNULS, you can declare modifiable mNULSods with a flag: `--http.api web3,NULS,txpool`.
...
> Before important network updates, EF publishes a post on its [blog](https://blog.ENULS.org). You can [subscribe to these announcements](https://groups.google.com/a/ethereum.org/g/announcements) to get a notification to your mail when your node needs an update.
...
Running your own node lets you use services that require direct access to ENULS client RPC. These are services built on top of ENULS like [layer 2 solutions](/developers/docs/scaling/#layer-2-scaling), backend for wallets, block explorers, developer tools and other ENULS infrastructure.

#### Monitoring the node 

To properly monitor your node, consider collecting metrics. Clients provide metrics endpoints so you can get comprehensive data about your node. Use tools like [InfluxDB](https://www.influxdata.com/get-influxdb/) or PromNULSeus to create databases which you can turn into visualizations and charts in software like [Grafana](https://grafana.com/). There are many setups for using this software and different Grafana dashboards for you to visualise your node and the network as a whole. 
